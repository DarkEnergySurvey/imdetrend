 ...
 * 06/05/2014: A. Drlica-Wagner <kadrlica@fnal.gov>
 *   Minor cleanup. Incorporated bugfix from Gary Bernstein.
 *   Changed defaults displayed to be actual defaults.
 *
 * $Id$ 
 *
 */
...
int mkbpm(int argc,char *argv[])
{
  ...
  if (!strncmp(&(argv[2][strlen(argv[2])-5]),".fits",5)
    ...
    // ADW: Shouldn't this be nbias not nflat???
    biasnames = (char **)calloc(nbias,sizeof(char *));
  ...
  for(j=0;j<nflat;j++){
    ...
    if(flat.mask && flag_mask)
      {
        // Bug fix from Gary Bernstein
	//for (i=0;i<output.npixels;i++)  output.mask[i] |= BADPIX_FLAT_MASK;
        output.mask[i] |= BADPIX_FLAT_MASK;
      }
  }
  ...
  printf("CCDNUM=%i\n",ccdnum);
  nbadcol = badcol("funky_column.lst",ccdnum,output);
...
//ADW: Should this be named "funkycol"?
int badcol(char *fname,int ccdnum,desimage out)
{
  ...
  //ADW: This should respect input file name
