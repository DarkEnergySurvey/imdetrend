...
int ImCorrect(int argc,char *argv[])
{
    ...
    char  comment[1000],longcomment[10000],filter[100]="",imagename[1000],
          firstimage[1000],retry_firstimage[1000],bpmname[1000],rootname[1000],
          varimname[1000],outputlist[1000],input_list_name[1000],
    ...
    while(1){
        ...
        case OPT_ILLUMINATION: // -illumination
            ...
            if(cloperr){
                reportevt(flag_verbose,STATUS,5,"Illumination correction image must follow -illumination");
                command_line_errors++;
                exit(1);
            }
            break;
...
/*      If no other corrections have been requested then the CCDNUM is not yet known
    ...
    for (im=0;im<imnum;im++) {
        ...
        /* ************************************** */
        /* RAG: Note this has been deprecated and removed in favor */
        /*      of the correction occurring in DECam_crosstalk     */
        /*      This comment remains to make clear that this is    */
        /*      where correction occurred along with updates to    */
        /*      DATASEC, TRIMSEC etc...                            */
...
/*      RAG need to add check for previous dark correction */
        ...
        if (flag_fast){
            retrievescale_fast(&output,scaleregionn,scalesort,flag_verbose,&scalefactor,&mode,&skysigma);
        }else{
            retrievescale(&output,scaleregionn,scalesort,flag_verbose,&scalefactor,&mode,&skysigma);
        }
        ...
        if ((flag_bias && flag_imbias)||
            ...
            for (i=0;i<output.npixels;i++){
                ...
                if (flag_bias && (bias.varim[i]>0.0) && (!(output.mask[i]&badpix_wgt0))){
            ...
            } /* Here ends the loop over pixels in an image */
        ...
        /* ********************************************************* */
        /* ***** remove source flux component in weight image  ***** */
        /* ********************************************************* */
...
#define INTERP_WIDTH  2
 ...
 // && flag_bpm) RAG 
        ...
        if (flag_updateskybrite || flag_updateskysigma) {
            ...
            if (flag_fast){
                retrievescale_fast(&output,scaleregionn,scalesort,flag_verbose,&skybrite_kv,&mode,&skysigma_kv);
            }else{
        ...
        if (flag_fringe){
            ...
            /* retrieve overall scaling from image */
            if (flag_fast){
                retrievescale_fast(&output,scaleregionn,scalesort,flag_verbose,&scalefactor,&mode,&skysigma_fr);
            }else{
                retrievescale(&output,scaleregionn,scalesort,flag_verbose,&scalefactor,&mode,&skysigma_fr);
            }
